...
class TensorFlowDKT(object):
    def __init__(self, config, is_add_loss=False):
        ...
        # 定义需要喂给模型的参数
        self.max_steps = tf.compat.v1.placeholder(tf.int32, name="max_steps")  # 当前batch中最大序列长度
        self.input_data = tf.compat.v1.placeholder(tf.float32, [None, None, input_size], name="input_x")

        self.sequence_len = tf.compat.v1.placeholder(tf.int32, [None], name="sequence_len")
        self.keep_prob = tf.compat.v1.placeholder(tf.float32, name="keep_prob")

        # 下面两个输入主要是为了计算当前的预测的结果和下一道题真实结果的损失
        # [1:]序列中的题目所属知识点ID
        self.target_id = tf.compat.v1.placeholder(tf.int32, [None, None], name="target_id")
        # [1:]序列的做题结果，可以看作真实的y值
        self.target_correctness = tf.compat.v1.placeholder(tf.float32, [None, None], name="target_correctness")

        # 下面两个输入主要是为了计算当前预测的结果和当前这道题真实结果的损失
        self.source_id = tf.compat.v1.placeholder(tf.int32, [None, None], name="source_id")
        self.source_correctness = tf.compat.v1.placeholder(tf.float32, [None, None], name="source_correctness")
        ...
        for idx, hidden_size in enumerate(hiddens):
            lstm_layer = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=hidden_size, state_is_tuple=True)
            hidden_layer = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell=lstm_layer,
                                                         output_keep_prob=self.keep_prob)
            hidden_layers.append(hidden_layer)
        self.hidden_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(cells=hidden_layers, state_is_tuple=True)
        
        first_layer = tf.compat.v1.layers.dense(self.input_data,self.input_size*0.85,activation = 'relu')
        middle_layer = tf.compat.v1.layers.dense(first_layer,self.num_skills * 2,activation = 'sigmoid')
        
        # 采用动态rnn，动态输入序列的长度
        outputs, self.current_state = tf.compat.v1.nn.dynamic_rnn(cell=self.hidden_cell,
                                                        inputs=middle_layer,
                                                        sequence_length=self.sequence_len,
                                                        dtype=tf.float32)

        # 隐层到输出层的权重系数[最后隐层的神经元数量，知识点数]
        output_w = tf.compat.v1.get_variable("W", [hiddens[-1], num_skills])
        output_b = tf.compat.v1.get_variable("b", [num_skills])
...
#             self.loss = self.get_cross_entropy_loss(flat_target_correctness, flat_target_logits)
            self.loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=flat_target_correctness,logits=flat_target_logits))
            if self.is_add_loss:
