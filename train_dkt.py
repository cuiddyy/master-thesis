...
class DKTEngine(object):
    ...
    def train_step(self, params, train_op, train_summary_op, train_summary_writer):
        ...
        _, step, summaries, loss, pred_all, binary_pred, pred, target_correctness = sess.run(
            [train_op, global_step, train_summary_op, dkt.loss, dkt.pred_all, dkt.binary_pred, dkt.pred, dkt.target_correctness],
            feed_dict)
        
#         print("params['input_x'][0]_len = ",len(params['input_x'][0]))
#         print("params['source_id'] = ",params['source_id'])
#         print("params['source_id'][1]_len = ",len(params['source_id'][1]))
#         print("params['source_correctness'] = ",params['source_correctness'])
#         print("pred_all[0][0]_len = ",len(pred_all[0]))       
    ...
    def dev_step(self, params, dev_summary_op, writer=None):
        ...
        step, summaries, loss, pred_all, pred, binary_pred, target_correctness = sess.run(
            [global_step, dev_summary_op, dkt.loss, dkt.pred_all, dkt.pred, dkt.binary_pred, dkt.target_correctness],
            feed_dict)

        for i_id,i_seq in enumerate(params['source_id']):
#             print("i_seq = ",i_seq)
#             print("i_id = ",i_id)
            for j_id,j_skill in enumerate(i_seq):
                if(j_skill == 0):
                    pd_y_pred_true_skill = pd.DataFrame(['#'])
                    pd_y_pred_true_skill.to_csv('./data_y_pred_true_skill.csv', mode='a', header=False, index=None)
                    break
#                 print(">>>>>",list(pred_all[i_id][j_id])+[params['source_correctness'][i_id][j_id]]+[j_skill])
                pd_y_pred_true_skill = pd.DataFrame([list(pred_all[i_id][j_id])+[params['source_correctness'][i_id][j_id]]+[j_skill]+[i_id]])
                pd_y_pred_true_skill.to_csv('./data_y_pred_true_skill.csv', mode='a', header=False, index=None)
    ...
    def run_epoch(self, fileName):
        ...
        with sess.as_default():
            ...
            with tf.name_scope("train"):
                with tf.compat.v1.variable_scope("dkt", reuse=None):
                    train_dkt = TensorFlowDKT(config, is_add_loss=config.is_add_loss)

            with tf.name_scope("test"):
                with tf.compat.v1.variable_scope("dkt", reuse=True):
            ...
            # 定义一个优化器
            optimizer = tf.compat.v1.train.AdamOptimizer(config.trainConfig.learning_rate)
...
#                     print("g>>>>>>",g,"v>>>>>>>>",v)
            ...
            # 训练时的 Summaries
            train_loss_summary = tf.compat.v1.summary.scalar("loss", train_dkt.loss)
            train_summary_op = tf.compat.v1.summary.merge([train_loss_summary, grad_summaries_merged])
            train_summary_dir = os.path.join(out_dir, "summaries", "train")
            train_summary_writer = tf.compat.v1.summary.FileWriter(train_summary_dir, sess.graph)

            # 测试时的 summaries
            test_loss_summary = tf.compat.v1.summary.scalar("loss", test_dkt.loss)
            dev_summary_op = tf.compat.v1.summary.merge([test_loss_summary])
...
#                               format(time_str, current_step, mean(losses), mean(accuracys), mean(aucs)))
                ...
                time_str = datetime.datetime.now().isoformat()
                print("epoch: {},dev: {}, step: {}, loss: {}, acc: {}, auc: {}".
                      format(i,time_str, current_step, mean(losses), mean(accuracys), mean(aucs)))
#             保存为pb模型文件
            builder = tf.compat.v1.saved_model.builder.SavedModelBuilder("./sevenSkillModel")
            inputs = {"input_x": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.input_data),
                      "target_id": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.target_id),
                      "max_steps": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.max_steps),
                      "sequence_len": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.sequence_len),
                      "keep_prob": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.keep_prob),
                      "batch_size": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.batch_size)}
            
            outputs = {"pred_all": tf.compat.v1.saved_model.utils.build_tensor_info(self.train_dkt.pred_all)}
...
#     fileName = "../data/assistments.txt"
    fileName = "../data/ASSISTments_skill_builder_data.csv"
    dktEngine = DKTEngine()
